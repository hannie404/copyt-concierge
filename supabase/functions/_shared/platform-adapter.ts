// Deno-side PlatformAdapter (Edge Functions). See also the Node-side twin at
// lib/platform-adapter.ts - two runtimes with different env-access
// mechanisms (Deno.env vs process.env), so this is deliberately duplicated
// rather than shared, matching the same interface.
...
const ENV_VAR_BY_PLATFORM: Record<string, string> = {
...
export function getAdapter(platform: string): PlatformAdapter {
  const envVar = ENV_VAR_BY_PLATFORM[platform];
  const apiKey = envVar ? Deno.env.get(envVar) : undefined;
